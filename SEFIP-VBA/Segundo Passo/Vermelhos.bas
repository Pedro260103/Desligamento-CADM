...
Sub Vermelho07()
    ...
    LocaldoArquivo = "X:\GESFO - ISO\GERINS em ES250-89\SEFIP\2021-RETIFICADORAS\07-2021-(2ª Retificadora)\CADM\SEFIP-CADM-(06.2021).RE"
    NumArquivo = FreeFile
    ...
    Do While Range("A" & Contador).Value <> ""
        ...
        If bmTXT = bm And DataAdimissaoTXT = DataAdmissao And Range("A" & Contador).Interior.Color = vbRed Then 'Or InStr(1, linhas(i), nome, vbTextCompare) > 0 Then  ' InStr(1, linhas(i), bm, vbTextCompare) > 0
            ...
            PisTXTCima = Mid(linhas(c - 1), 33, 11)
                ...
                Else
                    'If DataAdmissao = DataAdmissaoTXTCima And Pis = PisTXTCima Then ' posso tambem colocar o Pis para previnir erros de indentificação
                        'linhas(c) = NovoConteudo
                        'Range("A" & Contador).Interior.Color = vbBlue
                        'Exit For
                    'Else
                        'linhas(c) = linhas(c - 1)
                    'End If
